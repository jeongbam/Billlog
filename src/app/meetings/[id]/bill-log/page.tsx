...
function BillLogContent() {
  ...
  const nickname = (uid: string) => meeting.memberInfo[uid]?.nickname ?? "알 수 없음";
  ...
  return (
    <AppShell backHref={`/meetings/${id}`} title="Bill-log" backLabel="모임 상세로 돌아가기">
        <div className="pt-1">
          {receipts.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-[14px] font-bold">아직 등록된 영수증이 없어요</p>
              <p className="text-[12.5px] text-gray-500 mt-1.5">
                영수증을 등록하고 정산을 시작해보세요
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="text-[11px] font-bold text-gray-500 uppercase mt-5 mb-2">
                정산 현황
              </div>
              <Card className="mb-5">
                {settlements.map((s, i) => (
                  ...
                  >
                    ...
                    {s.status === "paid" ? (
                      ...
                      <button
                        ...
                        className="text-[11px] font-bold text-white bg-mint-300 rounded-full px-3 py-1.5 disabled:opacity-50"
                      >
                        완료로 표시
                      </button>
                    ) : isOwner ? (
                      <button
                        ...
                        disabled={busyUid === s.uid}
                        className="text-[11px] font-bold text-mint-500 border border-mint-200 rounded-full px-3 py-1.5 disabled:opacity-50"
                      >
                        알림 보내기
                      </button>
                    ) : (
              ...
              {receipts.map((r) => (
                <div key={r.id} className="flex items-center gap-2.5 py-2.5 border-b border-gray-100">
